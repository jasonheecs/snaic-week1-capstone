...
class ModelSelector:
    ...
    def _build_candidates(self, y: pd.Series) -> dict[str, object]:
        ...
        Three distinct algorithmic families (satisfies Part B requirement):
          ...
          3. GradientBoosting    — boosting (sequential tree ensemble); XGBoost
                                   is used instead if installed (faster, same family)
        """
        neg, pos = (y == 0).sum(), (y == 1).sum()

        # Family 1 — Linear
        # Logistic Regression is the correct linear model for classification.
        # (LinearRegression predicts numbers, not classes — wrong tool here.)
        ...
        candidates: dict[str, object] = {
            ...
            # class_weight='balanced' handles the 3:1 imbalance.
            ...
            # No class_weight param — uses sample_weight internally via sklearn's API.
        ...
        if _XGBOOST_AVAILABLE:
            ...
            # when installed. scale_pos_weight = neg/pos tells it how much harder to
            # penalise missing a churn customer vs a non-churn one.
